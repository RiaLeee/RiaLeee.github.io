...
*Commit과 Rollback 의 장점 : 데이터 무결성 보장, 논리적 연관된 작업 그룹핑, 영구적인 변경 전 데이터 변경 사항 확인 가능
...
*계층적 분류를 포함하고 있는 데이터 집계에 적합한 GROUP함수 : ROULLUP, CUBE

*절차형 SQL을 이용하여 주로 생성하는 것 : Procedure, Trigger, User Defined Function
...
*PL/SQL 의 사용 순서 ( "< 커서명 >" 으로 사용)
: 커서 선언 -> 커서 열기(Open) -> 커서 인출(Fetch) -> 커서 닫기(Close)

*절차형 SQL(PL/SQL 또는 T-SQL) : 복잡한 로직을 처리할 수 있는 강력한 도구
...
*데이터베이스의 동시성 제어를 위한 방법 : lock

*빌트인 함수(Built-in Function) : 별도의 정의없이 자동으로 생성되는 함수
*유저 정의 함수(User Defined Function): 사용자 정의 함수>> 프로시저와 함수로 구분됨.
...
2) 조인 조건이 잘못 설정되었을 때
  -ex) SELECT *
           FROM tableA
           JOIN tableB
           ON tableA.id = tableB.other_column;
3) 크로스 조인(CROSS JOIN) 사용 시
  -ex) SELECT *
           FROM tableA
           CROSS JOIN tableB;

*Intersect : 양쪽 모두 포함된 행을 검색 (중복된 행을 하나의 행으로 표시) >> 교집합

*차수(Degree) : 결과 릴레이션의 칼럼 수(+)
 ex. 학생 테이블에서 해당되는 컬럼 수 5개 + 학과 테이블에서 해당되는 컬럼 수 3개=8
*카디널리티(Cardinality): 선택된 행들의 개수(*)
...
*해시 조인: CPU 많이 사용 (*SQL 실행 계획에서 이너 조인으로도 받아들이기 가능)
...
*Natual 조인 : 두 테이블 간에 동일한 칼럼 이름을 가진 것만 출력하는 방법
-Alias(별칭부여) 불가능
...
*순수 관계 연산자 : Select, Join, Divide, Project

*PL/SQL 에서 변수에 값 넣기 : Select 칼럼명 INTO ~
...
*Distinct (중복제거)
*Decode(컬럼명, 지정 값, 컬럼명=지정값 같으면 해당 값, 다르면 해당 값)
*Coalesce(null이 아닐 경우 지정값, null일 경우 지정값, 2번째에서도 null 일 경우 디폴트값)
...
*DUAL 테이블 사용 시, 조회되는 데이터 타입이 다르면 실행X(오류 발생)

*LPAD(대상 문자, 지정길이, 채울문자)
...
*계층형 쿼리문의 내장 함수 (특정 값을 "검색" 할때 WHERE 절 사용)
...
*Rownum : Select문에서 행이 인출될 때 행에 부여되는 일렬번호
...
*집합 연산자(2개 이상의 테이블에서 조인없이 데이터 조회하는 것) 종류
-Union : 합집합, 중복 제거O
